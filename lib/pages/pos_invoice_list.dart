...
async{
  try{
   ...
   List<PaymentModeTypeAheadModel> payments =  await fetchSalesInvoicePaymentDetailsToReturn(invoice);
    Navigator.push(
    context,
    MaterialPageRoute(builder: (context) => CartItemScreen(discountAmount:invoiceDetails['discount'].toString() , discountPercent:invoiceDetails['additional_discount_percentage'].toString(), returnAgainst:invoiceDetails['name'],salesPersonID:invoiceDetails['sales_person'], customer:invoiceDetails['customer_name'], runInit: false, cartItems: invoiceItemDetails, isSalesReturn: true,)),
  );

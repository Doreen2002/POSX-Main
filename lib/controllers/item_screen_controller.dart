...
int  decimalPoints = UserPreference.getInt(PrefKeys.currencyPrecision) ?? 2;
...
Future<List<Item>> addItemsToCart(Item item, {from_hold=false}) async {
  try {
    ...
    {
      if ((cartItem.hasBatchNo == 0 && (cartItem.qty < (item.openingStock ?? 0))) || (cartItem.itemCode == item.itemCode && cartItem.hasBatchNo == 1 && cartItem.batchNumberSeries == item.batchNumberSeries && (cartItem.qty < (item.batchQty ?? 0))) ||  allowNegativeStock == 1 ) {
  ...
  try {
    ...
    if (amount.isNotEmpty || amount != '0.000') {
      ...
      singlediscountPercentController.text =
          (double.parse(amount) / originalRate * 100).toStringAsFixed(decimalPoints);
